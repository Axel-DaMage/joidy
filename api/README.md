...
FastAPI backend for Joidy — the main REST API that handles notes, tags, goals,
...
To run the API container alone:
...
Request flow follows **Routers → Services → Models**:
routers do only Pydantic validation, services hold business logic and DB ops,
models define the ORM schema.
